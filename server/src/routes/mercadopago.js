...
    payer_email: userEmail,
    // back_url: process.env.FRONTEND_URL || 'https://weedsmokersconnection.com/success', // Removido
    redirect_urls: {
    ...
    // O auto_recurring será definido pelo plano configurado no Mercado Pago
    // auto_recurring: {
    //   frequency: plan === 'anual' ? 12 : 6,
    //   frequency_type: "months",
    //   transaction_amount: plan === 'anual' ? 200 : 120,
    //   currency_id: "BRL"
    // }
  };
...
    const result = await preapprovalService.create({ body: preapprovalData });
    ...
    // O Mercado Pago agora retorna init_point diretamente na resposta
    res.json({ id: result.id, init_point: result.init_point });

  } catch (error) {
    // CORREÇÃO: Trata o erro de forma mais segura
    console.error('Erro ao criar assinatura:', JSON.stringify(error, null, 2));

    const errorMessage = error.message || 'Erro ao gerar assinatura';
